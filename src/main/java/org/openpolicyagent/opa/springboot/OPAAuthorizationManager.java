...
import static org.openpolicyagent.opa.springboot.input.InputConstants.ACTION_PROTOCOL;
import static org.openpolicyagent.opa.springboot.input.InputConstants.CONTEXT;
import static org.openpolicyagent.opa.springboot.input.InputConstants.CONTEXT_DATA;
import static org.openpolicyagent.opa.springboot.input.InputConstants.CONTEXT_HOST;
import static org.openpolicyagent.opa.springboot.input.InputConstants.CONTEXT_IP;
import static org.openpolicyagent.opa.springboot.input.InputConstants.CONTEXT_PORT;
...
import static org.openpolicyagent.opa.springboot.input.InputConstants.RESOURCE_ID;
...
import static org.openpolicyagent.opa.springboot.input.InputConstants.SUBJECT_DETAILS;
import static org.openpolicyagent.opa.springboot.input.InputConstants.SUBJECT_ID;
...
import org.openpolicyagent.opa.springboot.input.OPAInputContextCustomizer;
import org.openpolicyagent.opa.springboot.input.OPAInputResourceCustomizer;
import org.openpolicyagent.opa.springboot.input.OPAInputSubjectCustomizer;
import org.openpolicyagent.opa.springboot.input.OPAInputValidator;
 ...
 * "https://github.com/open-policy-agent/opa-springboot/tree/main/docs/reference/input-output-schema#input">input</a>
 * (map) based on {@link Authentication} and
 * {@link RequestAuthorizationContext}</li>
 * <li>sending an HTTP request with the input as the request body to the OPA
 * server</li>
 * <li>receiving the <a href=
 * "https://github.com/open-policy-agent/opa-springboot/tree/main/docs/reference/input-output-schema#output">output</a>
...
public class OPAAuthorizationManager implements AuthorizationManager<RequestAuthorizationContext> {
     ...
     *
     * @param opaClient           if null, a default {@link OPAClient} will be
     *                            created using {@code OPA_URL} environment variable
     *                            or default OPA url
     *                            ({@value OPAProperties#DEFAULT_URL}).
     * @param opaPath             if null, the default path defined by the OPA
     *                            configuration will be used, unless an
     ...
     * {@link AuthorizationDecision}, which can be
     * used to examine the OPA response. You should consider using the OPA Java SDK
    ...
    public OPAResponse opaRequest(Supplier<Authentication> authenticationSupplier, RequestAuthorizationContext object) {
        ...
        try {
            ...
            String selectedOPAPath = opaPathSelector != null
                    ? opaPathSelector.selectPath(authenticationSupplier.get(), object, input)
                    : opaPath;
    ...
    /**
     * If {@code nullableValue} is null, this function is a NO-OP, otherwise, it
     * calls
     ...
     * searched for in the {@link OPAResponse}.
     * A default value of {@value OPAProperties.Response.Context#DEFAULT_REASON_KEY}
     * is used. If the selected
     * key is not present in the response, the key which sorts lexicographically
